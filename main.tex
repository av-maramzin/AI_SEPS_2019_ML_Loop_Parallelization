...
\acmConference[AI-SEPS]{The 6th ACM SIGPLAN International Workshop on AI-Inspired and Empirical Methods for Software Engineering on Parallel Computing Systems}{2019}{Athens, Greece}
...
%
Thus, it improves over the traditional, profile-guided process by also taking into account the \emph{probability} that each of the profiled loops can be parallelized.
...
We approach the prediction of parallel loops as a \emph{supervised machine learning probabilistic classification problem}. Based on sequential reference applications and their manually parallelized counterparts as well as Intel's parallelizing C/C++ compiler, we create a data set of parallelizable and non-parallelizable loops. We extract loop features and use the data set to train a machine learning model, which links feature vectors describing the loops with their observed parallelizability. We then use the trained model as a probabilistic predictor: for each new loop we determine its feature vector and then predict the probability of the loop being parallelizable \cite{Niculescu-Mizil:2005:PGP:1102351.1102430}. For naturally probabilistic models like trees we directly use computed classification probabilities (fraction of parallelizable training samples in the leaf node). For the support vector machines classifier, we use Platt scaling to derive the probabilities.

In this section we introduce the parallelizability model, whereas Section~\ref{ml_predictive_performance} presents a common ML performance assessment done through accuracy, precision and recall scores. The latter gives us an additional degree of confidence in the predicted probabilities. Descriptions and definitions of the machine learning techniques we use can be found in the book by James \emph{et al.}~\cite{James:2013:ISL:2517747} and in the documentation of \textit{scikit-learn} library \cite{scikit-learn}. The latter is used in our work for all ML related tasks.
...
Table \ref{tab:average_accuracy_models} shows the overall predictive performance of different ML models measured with K-fold CV on the whole SNU NPB data set. Training and testing have been done for different values of K and as our experiments showed the accuracy remains stable across the entire range of K we studied (5, 10, 15, 20, 25, 30). The same is true of recall and precision scores.
\begin{comment}
\begin{figure}[ht]
...
The assistant we propose (Section \ref{practical_applications}) has been trained and tested using LOOCV, which is different from K-fold CV. Instead of treating the entire set of loops from all SNU NPB benchmarks as a single data set, we now train our model on nine benchmarks and test it on the remaining one. Doing so completely excludes the loops of a studied benchmark out of a training set, but allows us to get predictions for all benchmark loops, parallelize them if advised so, and test the effectiveness of our assistant. The drawback of the scheme is that it might potentially reduce the accuracy if the nature of loops in the studied benchmark dramatically differs from that of loops seen in the training set. Figure \ref{fig:accuracy_loocv_vs_kfold} compares LOOCV accuracy against that of K-Fold CV for all SNU NPB benchmarks. In that case K-Fold CV has been conducted on a data set consisting of loops from a single benchmark only. That comparison proves the presence of the training set incompleteness LOOCV method has. A richer training set than SNU NPB might potentially improve our ML model.
...
The loop ranking computed by our parallelization assistant combines a loop's contribution to overall execution time as well as its predicted probability of being parallelizable. In particular, we obtain the ranking by applying a shifted sigmoid function to the predicted parallelizability probability multiplied by the application runtime fraction a loop takes to run as shown in Figure \ref{fig:sigmoid_3d}.
\begin{figure}[ht]
\includegraphics[width=0.5\textwidth]{figures/product_func.pdf}
\caption{The ranking function combines for each loop its contribution to the application's execution time and its predicted probability of being parallelizable.}
...
\begin{table}
  ...
  \begin{minipage}{\columnwidth}
  \begin{center}
    \begin{tabu}{M{1.0cm}M{1.2cm}M{1.5cm}M{3.5cm}}
      \hline
      \rowfont{\bfseries}
...
classifying loops as parallelizable or not. There is a total of six possible classification outcomes that our scheme can 
produce (summarized in Table \ref{tab:combinations_table}). 

The most interesting cases are those where the ML predictor and the ICC Compiler disagree. This led us to
...
In our study we assume that the human expert starts with a sequential version of the SNU NPB benchmarks. The goal
is to parallelize these applications to a performance level matching those of their existing parallel versions.
By using our parallelization assistant we expect the human expert to consider fewer loops than by using a profiling-based approach, i.e.\ considering loops in the order of decreasing contributions to the overall program execution time. We also compare against the state-of-the-art fully automated parallelization approach implemented in the ICC Compiler. We notice that the ICC Compiler not only fails to achieve noticeable speedups, but actually slows down the performance on most of the benchmarks. For the BT benchmark the slowdown reaches 3.5 times. In contrast, the OpenMP reference implementation results in a (geo-)mean speedup of 2.19 across the benchmark suite.

Our results are presented as performance convergence curves in Figure \ref{fig:performance_convergence_line} interlinked with matching benchmark parallelization reports in Table \ref{tab:parallelization}. For each benchmark the curves plot its execution time (y-axis) as a function of the number of analyzed and possibly parallelized benchmark loops (x-axis). Dashed horizontal lines represent running times of the initial serial version and the target parallel version. The goal is to parallelize the benchmarks loop by loop and converge from a running time of the serial version to the best achievable time of the reference OpenMP implementation written by human experts as quickly as possible. Numerous convergence curves in the Figure \ref{fig:performance_convergence_line} demonstrate the parallelization process as guided by the rankings of different ML models. These curves compare model effectiveness and show that neural network based MLP (highlighed as a thick red line) works best. Assistant models compete against a profiler (thick dark blue line) to finish first.

In general, as it can be seen from the Figure \ref{fig:performance_convergence_line} the effectiveness of our assistant models either greater or equal that of a profiler. Benchmarks BT, CG and FT gets parallelized faster by following assistant rankings. In the cases of SP and UA benchmarks some of our assistants require more loops to be examined. However, as Table \ref{tab:parallelization} shows the loops of UA benchmark proposed by our assistants are actually simplier, since they consist of a fewer Lines Of Code (LOC) (508 and 579 of MLP and DT against 882 of profiler). If we calculate the average across all models LOC number our assistant would require a programmer to examine and compare that total number against that of a profiler, we will see a 20\% reduction.   

Let's recall a CG benchmark from a motivating example (Section 1). Table \ref{tab:parallelization} shows that the running time of its serial version is 69.38 seconds. SNU NPB provided parallel version takes 19.77 seconds to run. If we follow the ranking of MLP model and parallelize just a single small (6 LOC) loop from the Listing 2, we are going to shorten its execution time to 25.06 seconds (70\% of the maximum speedup). On the contrary, a profiler's ranking would require us to check 3 loops (the total of 330 LOC) to reach the same critical performance level. Full CG benchmark parallelization would bring us 19.77 seconds running time, but we can stop earlier.

There are some things in Figure \ref{fig:performance_convergence_line} worth explaining. In some cases (LU and UA) partial benchmark parallelization might actually slow down the performance. In the case of LU, after having parallelized the first 25 loops we do not converge to the best achievable parallel version performance. There are a total of 40 OpenMP pragmas in the benchmark and we need to parallelize all these loops altogether in order to reach the best performance level. In the case of UA benchmark we have a long-running innermost loop being suggested to be analyzed first by all rankings. Its parallelization actually kicks the running time up due to introduced at a wrong point synchronization barrier. It takes 30 loops for MLP model to achieve the parallel version performance here.
