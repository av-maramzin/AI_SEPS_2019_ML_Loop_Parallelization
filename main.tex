...
To avoid overfitting we discard irrelevant or redundant features. We run a pipeline of feature selection methods from the scikit-learn library. First we eliminate features with a low variance score, then we fit a decision tree-based model and select features with importance score above a given threshold. After that we repeatedly run \textit{Recursive Feature Elimination, Cross-Validated (RFECV)} to improve accuracy, precision and recall scores. This yields the final set of features. Table \ref{tab:best_features} presents the 10 highest ranked features in the set.
...
\begin{table}
  ...
  \vspace{-5mm}
\end{table}%
The SVC model has the highest average accuracy and has successfully managed to recall 95.24\% of all parallel loops. ICC succeeds to parallelize 812 out of 995 parallelizable loops. Thus, on average SVC extends ICC parallelization capabilities to 948 loops. Despite relatively high precision of 91.06\%, that extension comes at the price of some misprediction errors. Figure \ref{fig:prediction_stats} shows that unsafe (false positive) mispredictions dominate at 65\% of those cases where misclassifications occur.
...
We have compared the generated rankings of our parallelization assistant with the Intel \cpp{} Compiler, which due to its use of static analysis is conservative and occasionally misses some parallelization opportunities.
...
\begin{table}
  ...
  \end{center}
  \end{minipage}
  \caption{Possible outcomes for the side-by-side setup of our ML predictor and Intel's parallelizing compiler.}
  \label{tab:combinations_table}
\end{table}%

The ML approach to parallelization with a human expert responsible for final code transformation allows our parallelization assistant to be more aggressive than ICC. Hence, our model can predict more loops as parallelizable than ICC. The visible effect of this is that our assistant potentially assigns high probabilities to some possibly-parallel loops and awards them a relatively high rank, despite the fact that ICC is not capable of parallelizing them. In Table \ref{tab:combinations_table} these loops are classified under ``discovery".

We have set up an experiment where we apply our ML predictor side-by-side with ICC.  Both aim at independently classifyings loops as parallelizable or not. There are a total of 6 possible classification outcomes our scheme might potentially produce summarized in Table \ref{tab:combinations_table}. The most interesting cases are those, where the ML predictor disagrees with ICC. We then manually inspected these cases and investigated the cause of conservatism in ICC. 

A summary and breakdown by reason for ICC's failure to parallelize a loop can be found in Table \ref{tab:icc_missed_opportunities}.
\begin{table}
  \tabulinesep=2pt
  \begin{minipage}{\columnwidth}
  \begin{center}
  ...
  \end{center}
  \end{minipage}
  \caption{Classification of parallelizable loops rejected for parallelization by Intel's compiler.}
...
\begin{figure}[t!]
    ...
    \caption{
      Distribution of loop classification by ICC and our predictor. While there is agreement in about 80\% of the cases, our tool more aggressively discovers genuine parallelism in 10\%, while allowing false positives in 8\% of the inspected loops.}
...
In our study we assume the human expert to start with sequential C implementations of the SNU NAS benchmarks. The goal is to parallelize these benchmark applications to a performance level of the existing parallel versions of the benchmarks. By using our parallelization assistant we expect the human expert to consider fewer loops than by using a profiling-based approach, i.e.\ considering loops in the order of decreasing contributions to overall program execution time. We also compare against the fully automated parallelization approach implemented in the ICC compiler. We notice that the Intel compiler not only fails to achieve noticeable speedups, but actually slows down the performance on most of the benchmarks. For the BT benchmark the slowdown reaches 3.5 times. In contrast, the OpenMP reference implementation results in a (geo-)mean speedup of 2.19 across the benchmark suite.

Our results are shown as performance convergence curves in Figure \ref{fig:performance_convergence_line}. For each benchmark we present the execution time (on the y-axis) as the function of the growing number of analyzed and possibly parallelized loops (on the x-axis). Execution time is bounded to the top by the original sequential execution time, and to the bottom by the time for the reference OpenMP implementation written by human experts. In some cases (LU and UA) partial benchmark parallelization might actually slow down the performance. In the case of LU, we do not converge to the best achievable parallel version performance after having parallelized just the first 25 loops. There are a total of 40 OpenMP pragmas in the benchmark and we need to parallelize all these loops altogether in order to reach the best performance level.
